...
export type RoadPolyline = Array<[number, number]>  // [lat, lng] pairs

let _roads:      RoadPolyline[] = []
let _cacheLat  = NaN
let _cacheLng  = NaN
let _cacheHalf = 0
let _fetching  = false

export function getCachedRoads(): RoadPolyline[] {
  return _roads
}
...
export async function refreshRoads(
  ...
  if (!isNaN(_cacheLat) && drift < _cacheHalf * 0.3) return
  ...
  try {
    ...
    _roads = (data.elements ?? [])
      .filter((el: any) => el.type === 'way' && (el.geometry?.length ?? 0) >= 2)
      .map((el: any): RoadPolyline =>
        el.geometry.map((pt: any): [number, number] => [pt.lat, pt.lon])
      )
